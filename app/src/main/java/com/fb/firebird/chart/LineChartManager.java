...
public class LineChartManager {
    ...
    public LineChartManager(LineChart lineChart, float minYvalue, float maxYvalue) {
...
//        lineChart.setDescription(description);
        ...
        //保证Y轴从0开始，不然会上移一点
        leftYAxis.setAxisMinimum(this.minYvalue);
        leftYAxis.setAxisMaximum(this.maxYvalue);

        rightYAxis.setAxisMinimum(this.minYvalue);
        rightYAxis.setAxisMaximum(this.maxYvalue);
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<LineDataBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            LineDataBean data = dataList.get(i);
            /**
...
//        xAxis.setDrawScale(false);
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            @Override
            public String getFormattedValue(float value) {
                String tradeDate = dataList.get((int) value % dataList.size()).getKey();
                return FormatUtil.formatDateToMD(tradeDate);
            }
        });
...
//        });
     ...
     * 添加曲线
     */
    public void addLine(List<LineDataBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            LineDataBean data = dataList.get(i);
            Entry entry = new Entry(i, (float) data.getValue());
     ...
     */
    public void resetLine(int position, List<LineDataBean> dataList, String name, int color) {
